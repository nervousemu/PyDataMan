...
appDataPath = os.path.expanduser("~") + "/PyDataMan/"
...
class Main(QMainWindow, pyMainWindow.Ui_mainWindow):
    ...
    def import_action_triggered(self):
        ...
        # THIS IS HOMEWORK! Read python doc csv.reader ; There's nothing new here.

        dbFile = QFileDialog.getOpenFileName(parent=None, caption="Import database from a file", directory=".", filter="PyDataMan CSV (*.csv)")

        # if dbFile[0]:
        #     # try:
        #     #     with open(dbFile[0], newline='') as csvFile:
    ...
    def export_action_triggered(self):
        ...
        dbFile = QFileDialog.getSaveFileName(parent=None, caption="Export database to a file", directory=".", filter="PyDataMan CSV (*.csv)") + str('.csv')
